...
Security:
  ...
  - Optional VM backups (enabled via parameters.json)
...
if [ -z "$ENABLE_BACKUP" ]; then
    ENABLE_BACKUP=$(jq -r '.parameters.enableBackup.value // false' "$PARAMETERS_FILE")
fi

if [ -z "$BACKUP_FREQUENCY" ]; then
    BACKUP_FREQUENCY=$(jq -r '.parameters.backupFrequency.value // "Daily"' "$PARAMETERS_FILE")
fi
...
GATEWAY_TOKEN_GENERATED=$(openssl rand -hex 32)
...
if [ "$ENABLE_BACKUP" = "true" ]; then
    echo -e "${BLUE}VM Backup:${NC}           Enabled ($BACKUP_FREQUENCY, $BACKUP_RETENTION_DAYS days retention)"
else
    echo -e "${BLUE}VM Backup:${NC}           Disabled"
fi
echo ""
...
jq '{
  ...
  "parameters": (.parameters | with_entries(select(.key | IN(
    "location", "appName",
    "anthropicApiKey", "telegramBotToken", "telegramUserId",
    "enableBackup", "backupFrequency", "backupRetentionDays", "osDiskSizeGB"
  ))))
...
az deployment group create \
    ...
    --parameters gatewayToken="$GATEWAY_TOKEN_GENERATED" \
    --output table
...
GATEWAY_TOKEN="$GATEWAY_TOKEN_GENERATED"
CONTROL_UI_URL="https://$VM_DNS_NAME/?token=$GATEWAY_TOKEN"
...
if [ "$ENABLE_BACKUP" = "true" ]; then
    echo -e "${GREEN}✅ VM backups enabled ($BACKUP_FREQUENCY, $BACKUP_RETENTION_DAYS days retention)${NC}"
else
    echo -e "${YELLOW}⚠️  VM backups disabled (enable with enableBackup parameter)${NC}"
fi
echo ""
